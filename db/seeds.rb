...
def create_poster(name, filename)
  ...
  file = File.open(Rails.root.join("db/seeds/artworks", "#{filename}.webp"))
...
def create_illustration(name, filename, tags)
  ...
  file = File.open(Rails.root.join("db/seeds/artworks", "Poster_#{filename}.png"))
  artwork.cover_image.attach(io: file, filename: file.path)
  artwork
end
...
def create_photo(name, filename, tags, author: "Louis Sommer", year: nil)
  ...
  file = File.open(Rails.root.join("db/seeds/artworks", "#{filename}.jpg"))
  artwork.cover_image.attach(io: file, filename: file.path)
  artwork
end
